...
namespace nkentseu {
    namespace {  
        ...
        */
        std::string GetColorCode(Severity severity) {
            if (severity == Severity::Enum::Error) return "\033[1;31m";
            if (severity == Severity::Enum::Warning) return "\033[1;33m";
            if (severity == Severity::Enum::Debug) return "\033[1;34m";
            if (severity == Severity::Enum::Trace) return "\033[1;36m";
            if (severity == Severity::Enum::Critical) return "\033[1;35m";
            if (severity == Severity::Enum::Info) return "\033[1;32m";
            if (severity == Severity::Enum::Assert) return "\033[1;41m";
            if (severity == Severity::Enum::Fatal) return "\033[1;41m";
            return "";
        }
    }
    ...
    {
        std::string logType = loggerInfos.GetSeverity().ToString();
