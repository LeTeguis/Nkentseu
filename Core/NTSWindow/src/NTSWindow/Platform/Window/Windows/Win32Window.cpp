...
namespace nkentseu {
	...
	bool Win32Window::CreateWin32Window()
	{
		...
		m_Data = Memory::AllocateShared<WindowData>();

		if (m_Data == nullptr) {
			Lognts.Fatal("Cannot load window data");
			return false;
		}
		...
		if (!m_Data->Register(m_Properties.doubleClick, m_Properties)) {
			Log_nts.Fatal("Cannot register screen");
			return false;
		}
		...
		if (m_Properties.fullscreen) {
			...
			if ((size.width != screenSize.width) && (size.height != screenSize.height)) {
				if (ChangeDisplaySettings(&dmScreenSettings, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL) {
					...
					Lognts.Warning("Window stay in window mode");
				}
			}
		...
		if (m_Data->windowHandle == nullptr) {
			...
			Lognts.Fatal("Cannot create window");
			return false;
		}
