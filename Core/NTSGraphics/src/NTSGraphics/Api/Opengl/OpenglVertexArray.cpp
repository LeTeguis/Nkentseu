...
namespace nkentseu {
    ...
    OpenglVertexArray::OpenglVertexArray(Memory::Shared<Context> context, Memory::Shared<ShaderInputLayout> sil) : m_Context(Memory::Cast<OpenglContext>(context)) {
    ...
    bool OpenglVertexArray::Draw(RenderPrimitive::Enum primitive)
    {
        if (m_Context == nullptr || m_VertexArrayObject == 0) return false;
        ...
        uint32 prim = GLConvert::GetPrimitiveType(primitive);
        ...
        if (m_IndexBuffer != nullptr) {
            ...
            glCheckError(first, result, glDrawElements(prim, m_IndexBuffer->Leng(), indexType, 0), "cannot draw elements");
        }
        else {
            glCheckError(first, result, glDrawArrays(prim, 0, Leng()), "cannot draw arrays");
        }
    ...
    bool OpenglVertexArray::BindIndex()
    {
        if (m_Context == nullptr || m_VertexArrayObject == 0 || m_BindInfo == BindInfo::INDEX_BIND) {
            if (m_Context == nullptr) gLog.Debug();
            //if (m_VertexArrayObject == 0) gLog.Debug();
            if (m_BindInfo == BindInfo::INDEX_BIND) gLog.Debug();
            return false;
        }
