...
#include "NTSCore/System.h" // Include header for System class (assuming this is needed)
#include "NTSLogger/LoggerInfos.h"  // Include header for LoggerInfos class
...
namespace nkentseu {
    ...
    public:
        ...
        void Info(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Info, format, args...);
        }

        template<typename... Args>
        void Asserts(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Assert, format, args...);
        }

        template<typename... Args>
        void Error(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Error, format, args...);
        }

        template<typename... Args>
        void Warning(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Warning, format, args...);
        }

        template<typename... Args>
        void Debug(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Debug, format, args...);
        }

        template<typename... Args>
        void Trace(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Trace, format, args...);
        }

        template<typename... Args>
        void Critical(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Critical, format, args...);
        }

        template<typename... Args>
        void Fatal(const char* format = "", Args&&... args) {
            Log(Severity::Enum::Fatal, format, args...);
        }
        /**
