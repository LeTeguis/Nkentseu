...
namespace nkentseu {
    ...
    OpenglShaderInputLayout::OpenglShaderInputLayout(Memory::Shared<Context> context) : m_Context(Memory::SharedCast<OpenglContext>(context)) {
    }
    ...
    void OpenglShaderInputLayout::ResetConstantPush(OpenglShader* shader)
    {
        ...
        for (auto& [name, buffer] : m_PushConstants) {
            ...
            if (blockIndex == GL_INVALID_INDEX) {
                Log_nts.Error("Error: Uniform block index is invalid : {0}-{1}", buffer.name, shader->GetProgramme());
            }
            else {
